 ...
 * the tslib specifiers in the packed tarballs, which is the emit itself rather
...
/**
 * The value tsc resolves for a config, reading the `extends` chain nearest-first
 * the way tsc does. A base that cannot be located is an error rather than a
 * missing value: a guard that silently reads `undefined` for an option set two
 * levels up asserts nothing.
 */
...
function declaredImportHelpers(config: string): boolean | undefined {
  ...
  // tsc applies later entries in an `extends` array over earlier ones, so the
  // nearest definition is found by reading the list backwards.
 ...
 * `tsc --build` over one. A project built by anything else emits its JavaScript
 * through a bundler, which resolves the helper at build time and leaves no
 * import behind.
 */
...
describe('tslib dependencies', () => {
  ...
  it('are absent from every released package whose JavaScript is bundled', async () => {
    // A bundler inlines the helper into the output it writes, so the published
    // package has no tslib import to resolve and nothing for the declaration to
    // cover.
    const bundled = (await releasedProjects()).filter(
      (project) => !tscBuildConfig(project),
    );

    expect(
      bundled.length,
      'no released package is bundled -- this check is matching nothing',
    ).toBeGreaterThan(0);

    expect(
      bundled
        .filter((project) => TSLIB in (manifestOf(project).dependencies ?? {}))

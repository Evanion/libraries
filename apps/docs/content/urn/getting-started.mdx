...
RFC 8141 equivalence is case-insensitive on the scheme and the NID, and exact
on the NSS:
...
function safeUrnOperation<T>(operation: () => T, fallback: T): T {
  try {
    return operation();
  } catch (error) {
    if (error instanceof InvalidError) {
...
You're now ready to start using URNs in your project! 🎉

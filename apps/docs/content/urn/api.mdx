...
class URN {
  ...
  static readonly isValid: RegExp;
}
```
...
Returns `true` when the string has at least three non-empty parts. Never throws,
so use it to test input before parsing.

### `extractId(urnString)`

Returns everything after the scheme and the NID. Throws `ValidationError` on
malformed input.

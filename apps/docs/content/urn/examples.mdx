...
Worked patterns. Every stated result is what the library actually returns —
including the ones that are not what you would first guess.
...
That is the property to lean on when a payload mixes namespaces:
...
When the set of namespaces is open-ended, a single class and a positional NID is
less ceremony than a subclass nobody reads:
...
What gets skipped is narrower than you might expect. The NSS grammar is RFC
3986's `pchar`, which includes `!`, `$`, `&`, `'`, `(`, `)`, `*`, `+`, `,`, `;`,
`=`, `:` and `@`:

...
An even size is not a stylistic preference. The fold applied at every second
position is `g(x) = floor(2x / n) + (2x mod n)`, and for an odd `n` that map is
not injective — over a three-character dictionary, `g(1)` and `g(2)` are both 2.

...
Node 20 or newer. ESM only. No dependencies.
...
Nothing is checked at use. An instance you hold cannot produce a token its own
`validate` rejects, because every constraint a dictionary has to satisfy was
settled before the object existed.

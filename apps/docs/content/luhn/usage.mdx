...
Type the order number below back in and watch the verdict. The grouping and the
case make no difference to it, because both are filtered before anything is
computed; break the code with either button and it is refused.

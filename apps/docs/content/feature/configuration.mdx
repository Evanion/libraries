...
| `variants`          | the named variants this feature splits across. See [Variants](/feature/variants) |
...
it matches on, so the rule keeps its name when another rule is inserted above
it and when an operator moves a ramp. Name a rule you expect to read in a log:
an operator reads `staff-only` faster than a hash.

The hash excludes `rollout.percent`, so two rollout rules on one feature whose
conditions, `by` and `seed` all match derive the same id. Give such rules
explicit ids, or a breakdown carries two entries under one name.

A rule with neither `when` nor `rollout` matches unconditionally.

A rule on a feature that declares variants can carry `variant`, naming one of
them. A matching rule with `variant` set pins that variant on the decision,
overriding the weights entirely. See [Variants](/feature/variants).
...
//   failed: { field: 'role', op: 'ne', value: 'customer' } }] }
```

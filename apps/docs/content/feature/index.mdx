...
dependant's users — which is the failure this package exists to prevent.

## Not published yet

The package carries `private: true`. A release run versions and tags it without
publishing, because npm cannot configure a trusted publisher for a package that
does not exist on the registry, so the first version has to go up by hand.
Inside this repository it resolves from source like any other workspace package.
...
"the system turned it off" — which is the distinction an operator needs at 3am.

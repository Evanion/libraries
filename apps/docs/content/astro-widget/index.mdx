...
- [Getting started](/astro-widget/getting-started) — installing, the registry, the data shape, chrome
- [Validation](/astro-widget/validation) — catching a bad CMS save at build time
- [API reference](/astro-widget/api)

## Where to start with astro-widget

The registry is what maps an item's `type` to an `.astro` component, and
`@evanion/astro-widget` reads nothing else to decide what a section renders.
[Getting started](/astro-widget/getting-started) installs the package and builds
that registry.

...
controls. The trusted side, a loader, a server component, an action or a route
handler, calls `@evanion/acl` and makes the decision that counts.

**What you get.** A line through your app: the side that fetches and writes
decides, and the side that renders toggles. A hook never appears on the first
side.

**Why you want it.** `useCan` and `access.can` both compile, both return a
`Decision`, and neither call says which side enforces. A shopper can work around
the browser's answer and cannot touch the handler's.

**How the library gets you there.** `'use client'` sits at the top of the
package's entry point, so a hook cannot be called on the trusted side at all.
...
The server never reads that copy. Every loader, every action and every handler
evaluates its own.

Treat the browser's answer as a convenience for toggling what a control looks
like, and nothing else. A shopper shown an Edit button that 403s has been
misled, and the hooks exist to prevent that. Everything that protects the

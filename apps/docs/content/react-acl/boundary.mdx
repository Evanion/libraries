...
**The concept.** `@evanion/react-acl` toggles controls in the browser.

**What you get.** The side that writes decides.

**Why you want it.** A shopper can work around the browser.

**How the library gets you there.** `'use client'` keeps hooks off the server.
...
`@evanion/acl` and makes the decision that counts. The side that fetches and
writes decides, the side that renders toggles, and a hook never appears on the
first side. `'use client'` sits at the top of the package's entry point, so a
hook cannot be called on the trusted side at all.
...
The server never reads the browser's copy. Every loader, every action and every
handler evaluates its own.

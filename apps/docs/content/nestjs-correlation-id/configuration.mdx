...
One to 128 word characters, dots, colons and hyphens — RFC 9110 token
characters, plus the colon. It excludes:
...
Tightening it to the exact shape your stack mints is safe. A validator that
accepts CR or LF accepts response splitting and log forging, and one with no
length bound lets a single request write an arbitrarily long string into every
log line it touches.

...
By the end of this page a page renders a widget region from an item array, and the
type checker rejects an item naming a component the map does not have.
...
### The markup one item produced

`renderToStaticMarkup`, from `react-dom/server`, gives the HTML string React wrote
for one component and one item:

```tsx twoslash file=libs/react-widget/README.md region=region-markup

```

This factory passes no `chrome.wrapper`, so the region takes the default `section`
the `catalogue` fence above replaced with `<section className="shelf">`. Inside the
wrapper each widget sits in an element carrying the item's `id` and `type`, so a
widget is findable in the DOM without the widget writing the attributes itself.

...
**What you get.** The seven steps written out, and which of the four outcomes
each one produces.

**Why you want it.** No option reorders them, so a rule you read in one service
decides the same way in every other.
...
[The Decision object every check returns](/acl/decision-object) has the shape each step hands back:
...
- [The Decision object every check returns](/acl/decision-object) — the decision object, `unevaluable` as a third
  state, and the clock

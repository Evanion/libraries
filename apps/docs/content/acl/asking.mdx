...
**The concept.** `can` answers one question with a decision.
**What you get.** One answer the handler gates on.
**Why you want it.** A hand-written check drifts.
**How the library gets you there.** `can` decides one row, `canMany` a page.
...
Write the check by hand and you write it twice, once in the handler that refuses
the save and once in the component that greys out the button. The two drift, and
the customer sees an enabled button that 403s.

`authorize` binds the subject so no handler restates it, and
`access.object(kind)` binds the object kind. `can(subject, kind, action,
object?, now?)` takes the object kind in `kind`, so you pass `'question'` and
...
The two handles are alternatives, so bind the axis the call site repeats:

- `authorize` binds the subject and one instant. With no `now` the instant is
  the moment you call it, and `authorize(subject, { now })` names the instant
...
- `access.object(kind)` binds the other axis. The handle names one object kind
  once and returns `can`, `canMany`, `canFields` and `readsObject`, each of
  which still takes the subject first.

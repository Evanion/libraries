...
**The concept.** Express is trusted end to end.

**What you get.** Every handler calls `can` itself.

**Why you want it.** `app.use` is positional, so registration order mediates.

**How the library gets you there.** `@evanion/acl` has no Express adapter.

## What an Express app with `@evanion/acl` holds

An Express app hands no client tree anything, so every decision it makes is
authoritative and the only question is whether the decision happens at all. This
page builds an API over Baize's catalogue out of three pieces:

- one matrix at module scope,
- one middleware binding the subject onto `res.locals`,
- a `can` call in every handler that touches a title.
...
`app.use` is positional, so a route registered above the middleware that binds

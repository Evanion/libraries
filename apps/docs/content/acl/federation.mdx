...
**The concept.** `federatedPolicies` holds one `Access` per origin.

**What you get.** A view routes each key home.

**Why you want it.** Two services mean different rows by `listing`.

**How the library gets you there.** `federatedPolicies` routes by canonical key.
...
two databases with two field sets. A merged view answers about a row that does
not exist.

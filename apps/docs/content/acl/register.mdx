...
Thirty-six classes of attack, and what `@evanion/acl` does about each. The suite
in `libs/acl/src/security` holds one test per entry, under the entry's
identifier, and runs under `nx test @evanion/acl`. Each row names the Common
Weakness Enumeration (CWE) and the Open Worldwide Application Security Project
(OWASP).

[The security contract](/acl/security) argues the same material in prose.

## The three tiers

Each entry sits in one tier, and the tier decides what its test may claim.

In tier 1 the library prevents the attack, and the test asserts that it raises a
construction error or reaches a refusal. In tier 2 the primitive exists and the
consumer calls it. The test there asserts that the primitive is correct and that
the idiom it replaces is wrong on the same decision. In tier 3 the class is
structurally out of scope: the contract clause exists, and where the behaviour
reads like a defence, the test pins what the engine really does.

A tier 3 test asserts no defence; a pass implies one.
...
`libs/acl/src/security`, matched by the package's own `src/**/*.{test,spec}.ts`
include, so CI runs it with no project to forget. A unit test asserts what a
function does; a register test asserts that an attack fails.

...
Thirty-six classes of attack, and what `@evanion/acl` does about each. The
suite in `libs/acl/src/security` is checked against this register: one entry per
class, one identifier, one test, run by `nx test @evanion/acl`.
...
consumer calls it, and the test asserts that the primitive is correct and that
the idiom it replaces is wrong on the same decision. In tier 3 the class is
structurally out of scope and the test asserts nothing: the contract clause
exists, and where the behaviour reads like a defence, the test pins what the
engine really does.

A tier 3 test asserts no defence; a passing one would imply one.
...
include, so CI runs it with no project to forget. It sits apart from the unit
tests: a unit test asserts what a function does, these assert that an attack
fails.

...
shop lists the title. A guard called with no title answers `unevaluable`, and an
app reading that as a grant lets an operator in Gothenburg edit a Stockholm
listing.

**How the library gets you there.** `access.readsObject(key, action)` tells the
guard which permissions it can finish, so the guard refuses those and passes the
others on. `canFields` and `pickAllowedFields` then close the write in the
service.

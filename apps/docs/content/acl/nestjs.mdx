...
**The concept.** A Nest request passes a guard before the row is fetched and a
service after it, so one write decides twice.
...
Six permissions, each carrying `visibility` except `inventory.read`, which
...
- [Why deny, then allow, then deny?](/acl/resolution) — where the guard's two
  branches come from
- [The Decision object every check returns](/acl/decision-object) — the `unevaluable` state `readsObject` exists for

...
The test-kit entry point. `@evanion/acl` re-exports none of it, so an
application importing the engine pulls none of it into its bundle, and nothing
here imports a test framework. See [Testing a contract I
consume](/acl/testing).
...
key answers `unknown-action` and lands in `changed` rather than throwing
...
one the producer publishes now. Settling `now` once compares a time-windowed
permission at one epoch rather than at two readings of the wall clock.
...
The seam [`diffMatrix`](#diffmatrix) drops into. `contractDrift` reads nothing
out of the result and carries it onto `report.diff`, so the diff's own shape
stays the diff's business and arrives here as `D`.
...
stating no `maxStale` raises `MissingFreshnessBudgetError` here rather than at
...
A field the decision carries no entry for is reported apart, because the maps
are keyed by the names the document declares and an absent name means the test
named a field the permission never had.
...
is, so a false one has at least two places to look and the maps are both of
them.
...
The base class every refusal in `@evanion/acl/testing` raises. A plain `Error`
subclass and no matcher, so vitest, `node:test` and jest each report a throw
from a test body the same way.

...
`ruleId` is the identity `diffMatrix` indexes a rule by.

```ts signature
function ruleId(rule: Rule, side: 'allow' | 'deny'): string;
```

An author-supplied `rule.id` wins. Otherwise the id is the side and a hash of
the rule's canonicalised conditions, so a rule keeps its id across a reorder and
an insertion.

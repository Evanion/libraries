...
`fetchedAt` is when this holder last validated, and supplying it obliges the
document to state `maxStale`, which here is a local ceiling `min`-ed with the
document's.
...
`subject.*` path is a definite miss instead.
...
subject, so none of the four is here.

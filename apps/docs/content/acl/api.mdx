...
`Actions` is the block parameter `.for()` hands you. `Vocab` is what this kind
may declare, `Act` what this chain declared, and only `Act` reaches the
permission keys.
...
`Cond` is an opaque condition tree, and `build()` flattens one to a rule per
`or` branch.

...
**Why you want it.** A consumer that cannot read your rules writes its own copy,
which your team does not own and cannot correct.
...
re-evaluates on its own matrix on every call and refuses. A stale consumer draws
a control the owner revoked, and the write behind that control still fails at
orders.

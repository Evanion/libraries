...
one character is exactly the thing this package exists to avoid. Set `chunkSize`
equal to `length` for an unchunked code.
...
`length - 1` characters are drawn from `crypto.randomBytes` with `byte % n`,
...
It returns a reason rather than throwing, because it is the cheap gate in front
of a lookup rather than an exceptional path. It is total and free of side
effects, which makes it safe to run on every request before doing anything
expensive.

...
`crypto.randomBytes` yields 0–255, and `byte % n` over-represents the first
...
generation variable-time. The constraint is satisfiable — the default proves it —
so the constraint is what this package takes.
...
Building an alphabet is four simultaneous constraints, and every error names the
one you broke. Call `createToken` once in a test rather than reasoning about it.

...
| `MISSING_FIELD`    | `missing field <name>`      | a prop the caller's `required` map demands is absent or blank |
...
## An absent props is a problem, not an empty one
...
## Where to run the widget check

No renderer calls `validateItems`. Each renderer stays defensive, skipping an
item it cannot draw and warning about it, so one bad save never takes a running
page down. This is the loud gate, and you choose where it sits:

- **At ingestion.** A webhook takes the CMS save, calls `validateItems`, and
  answers with the problems. The editor finds out while the page they were
  editing is still open.
- **In CI.** A test walks the committed payloads and fails on a non-empty
  report. The check costs nothing and it runs on every pull request.
- **In a build script.** The build fails before the pages are written, which is
  the last point at which a hole in a page is still cheap.

None of those needs a renderer. `known` may be a plain list of names, so a
script validates a payload without importing a single component it will never
draw.

## Why a type is looked up as an own key

`type` is matched with `Object.prototype.hasOwnProperty` rather than with `in`
or a bare index. Items are CMS data, so any string is reachable, and both of
the looser forms walk the prototype chain: an item typed `constructor`,
`toString` or `__proto__` would otherwise pass as a registered type, and the
value that came back would be a function off `Object.prototype`. The same rule
governs the `required` map's own lookup.

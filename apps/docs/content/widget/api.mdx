...
[Getting started](/widget/getting-started) teaches the item shape and
[What it reports](/widget/problems) carries the message catalogue, and this
...
call it, neither renderer re-exports it, and no example on this page runs it,
because an example that proved the second call stayed quiet would be a test
...
same message. Each renderer's test setup calls this before every test.
...
```ts file=libs/widget/README.md region=warning

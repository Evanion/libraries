...
**What you will have.** A payload for the Baize shop's counter view, a registry
naming the types it may use, and a check that fails your build when a bookseller
saves a widget type nothing draws.

**Why you would want it.** A CMS lets someone rename a section type on a
...
```ts file=libs/widget/README.md region=shape
...
`meta` is placement: which column the item sits in, what it spans, whether a
rule runs above it. It goes to the chrome, which is the markup a renderer wraps
each item in, and never into the item's own props, because where a widget sits
...
```ts file=libs/widget/README.md region=registry
...
`validateItems` walks the array, recurses into `children`, and returns
everything it found. It throws nothing:

```ts file=libs/widget/README.md region=unknown
...
## Move an item and watch the region recompose

The array is the page, which is a claim you can put your hands on. Below is the
shop counter's back-office view, composed from seven items, with the items
themselves listed underneath and a pair of move controls in the gutter of every
...
built around, and the control is the same component the front page opens with.

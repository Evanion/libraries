...
A widget set is a component map, a list of items, and the `Widgets`
component that renders one against the other.
...
// 2. The set. Call createWidgets once, at module scope.
...
a variable it is not: `const items = [{ type: 'news', … }]` infers
...
renderer stays defensive: a malformed item, an unknown `type`, or a non-array
`children` is skipped with a development-only `console.warn` rather than taking
the page down.
...
`Widgets` never calls it. Validation is an explicit gate; the renderer is the
net underneath it. See [the API reference](/widget/api#validateitems) for what
it reports.

## Next

- [Chrome, `meta` and Suspense](/widget/advanced) — the composition seams
- [API reference](/widget/api) — every export

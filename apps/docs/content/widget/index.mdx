...
**What you will have.** One item array that renders the same widgets in the
...
**Why you would want it.** Both renderers re-export everything below, so most
readers already have this package and never type its name. What the core is
...
**How the library gets you there.** Three exports carry it. `AnyWidgetItem` is
...
- You are drawing a page. Install `@evanion/react-widget` or
  ...
  the item model, so adding the core by hand buys a second version to keep in
  step.
...
Node 20 or newer, ESM only, and no dependencies. There is no `require`
condition in the exports map, so a CommonJS consumer gets `ERR_REQUIRE_ESM`.

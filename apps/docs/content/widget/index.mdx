...
That constraint shapes the whole API. There is no provider, no hook and no class
error boundary here, because `react/package.json` maps the `react-server`
condition to a build that does not export the APIs those need. Everything that
would have been context is a prop: `ctx` for page-level data, `meta` for
placement.
...
- [Getting started](/widget/getting-started) — install, the component map, typing items
- [API reference](/widget/api) — every export, every prop, every type
- [Chrome, `meta` and Suspense](/widget/advanced) — the composition seams
- [Examples](/widget/examples) — worked regions
- [Playground](/widget/playground) — run it in the browser
- [Migrating from 0.1.x](/widget/api#migrating-from-01x) — the RSC redesign

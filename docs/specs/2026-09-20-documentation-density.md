...
`apps/docs/content/feature/*.mdx` edits that sat beside it on that branch. This
paragraph claimed it had reached `main` on 2026-09-24 while it was still sitting
on `docs/prose-density-spec`, so for a day the file asserted its own merge before
the merge happened. It landed on `main` as `4cdad1c` on 2026-09-25. The date is
corrected here rather than deleted, because the next document to write its own
merge into its header should know this one did it and was wrong for a day.

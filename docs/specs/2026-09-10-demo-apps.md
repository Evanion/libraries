...
### `apps/storefront-rsc` — Next App Router, minimal
...
- **Complexity** — a five-stop sequential ramp, because complexity is ordinal.
  The ramp is the card's one saturated data signal: it sets the game's title, the
  way an item name carries its rarity, and the five stops carry the five tier
  names — Gateway, Light, Midweight, Heavy, Brain-burner.
- **Availability** — in stock, preorder, reprint pending, out of print. State
  pills. A state, not a ladder, which is why it keeps its own colours.
- **Mechanism** — categorical. Worker placement, deckbuilder, area control,
  co-op, dexterity. Shape, wording and the link to its category on a card, and a
  hue only where the mechanism is itself the subject: the category index and a
  category page's own heading.

The colour ladder carries one thing, and complexity is that thing. A card spends
one saturated colour plus the box art, which is what rations attention; a card
that hued its mechanism as well gave a reader two scales to learn and no reason
to prefer either.
...
complexity is how anyone identifies a game at a glance, so it leads rather than
sitting beneath a photograph. The complexity cell shows the tier word and the
rating under it: the word is what a shopper reads and what carries the ladder's
meaning without colour, and the number is what separates two Midweight games.

```
┌────────────────────────────────────────┐
│ WINGSPAN                    ● IN STOCK │  title on its complexity rung
│ engine building · set collection       │  tags, named and unhued
├────────────────────────────────────────┤
│  1–5      40–70 min      Midweight     │  the tier leads
│  players  playtime    complexity 2.4/5 │  the rating follows it
├────────────────────────────────────────┤

...
10. `file=… region=…` is the only way a TypeScript example reaches a page. A
    ...
    reference. Section 5.
11. An interactable example is how a concept is introduced, not an illustration
    attached to it afterwards. A teaching page that introduces a concept puts the
    control before the prose, and where that is not achievable the page says so
    rather than shipping a toy. Section 5.
12. Every section ships at least one control a reader can operate, on the
...
13. One domain across all nine sections: the board game shop the demo apps
...
14. The region loader learns `// #region` markers in `.ts`/`.tsx` sources, so a
    ...
    Section 10.
15. Separators, never folders, at every level. `acl/integrations/` is the one
    folder on the site and it is wrong; it becomes five pages under a
    `Platforms` separator. Section 8.
16. Every section directory has a `_meta.ts` naming every page in it, in reading
...
17. Nothing runnable exists only on the front page. Every landing specimen has a
    counterpart inside the section it advertises, and `AccessDemo` moves into
    MDX. Section 9.
18. An API reference's signature blocks are generated from the package's emitted
...
19. Nine guards in `tools/repo-checks` (section 12). Seven of them fail today;
    the other two have not been run. Decisions 1 to 3 are the one substantial
    part of this standard with no guard behind it, and section 12 says so.
20. Nothing here is retrofitted in one pass. The order is section 13.

Decision 13 is the expensive one: it re-themes nineteen doctested regions, a
...
pages a defect. Decision 14 is the only one that needs code outside `apps/docs`.
...
`_meta.ts` order is the sequence, which is why decision 16 makes that file
...
across a section boundary instead of resetting at each one. Section 5's
interactable examples are what the reader uses to become familiar with a concept
...
called 'API'." That is decision 15's argument arriving from outside this repo.
...
`validation`. It is where decision 12 lands, and it comes before
...
`integrations/`, currently expressed as a folder. Under decision 15 that becomes
...
under decision 18 most of its length is generated rather than written. The
...
The three requirements are decision 11 and are new: they are the teaching pages,
and a teaching page is where a concept is introduced. A section is not obliged to
build three separate controls — one control that a later page re-enters with a
different seed is the same control and counts — but a teaching page that introduces
a concept with nothing but prose and a fence is incomplete, and needs the
exemption below rather than silence.
...
### The interactable example is the method

A control is not an illustration hung on a finished page. On a page that
introduces a concept it is how the concept is introduced, and the page is built
in this order:

1. **The thing, rendered.** The reader sees the consequence before reading a
   word about it.
2. **The reader's hand on it.** One editable dimension, and it is the concept —
   not a parameter beside the concept. A page introducing "a deny beats an
   allow" makes the deny the thing that is clicked. A page introducing the field
   axis makes a field the thing that is toggled. If the control's editable
   surface is not the page's one concept, the control belongs on a different
   page.
3. **The prose.** It explains what the reader has just watched happen, in the
   past tense of their own action. A page whose prose announces what is about to
   be shown has the order backwards and has written a caption.
4. **The code**, as an executed region, showing the call that produced what they
   were just holding.

This is a stronger claim than decision 12's floor, which only asks each section
for one control. It says the control is the explanation, and that the fence
underneath it is evidence rather than instruction.

**Where it is not achievable, and the page says so.** Pretending otherwise
produces a toy control, and a toy is worse than the paragraph it replaced
because it costs a reader the time to discover it teaches nothing. The cases,
named:
...
against an asserted `@ts-expect-error`, which is the closest a static export
gets. It is evidence rather than interaction, and the page calls it that.
...
calculator, and a calculator is legitimate here precisely because the number
**is** what the reader came for. The boundary is the same one the demo rules
draw below — render the output where the output is the product.
...
A page failing all three is better as prose and one executed region, and it
takes the exemption above.
...
Three rules follow, and they bind every control added under decisions 11 and 12:
...
1. **Consequence first, code second.** The reader sees the interface change
   ...
   This is step 1 of the method above, and the method is this rule generalised
   from a landing specimen to a page.
...
5. If it introduces a concept, it carries a control, placed before the prose
   that explains the concept — or it carries one of section 5's named
   exemptions, stated on the page.
6. Every example in it is set in the shop, or the section carries a recorded
   `domainExempt`, or it is a reference page taking section 6's minimal-example
   allowance.
7. Every TypeScript fence is a `file=… region=…` reference or carries an
   exemption tag.
8. Every region it references exists and executes in the package's test run.
9. It is under 1,200 words of prose, counting neither fences nor generated
   signatures.
10. It is one kind of page from section 4, and the Diátaxis diagnostic in
    section 3 does not fire on it.
11. If it is the demonstration page, it mounts the section's control.
12. Every `@evanion/…` symbol it names is exported from that package's entry.
13. Every internal link it makes resolves to a page that exists.
...
**Must appear in both:** every runnable control. This is decision 17 and it is
...
5. **`type-checking.mdx` under decision 14.** This is the interesting one, and
   ...
   Decision 14 closes that: the region parser learns `// #region name` and
...
4. **The control is `apps/storefront`.** The demo app already renders this
   ...
   section exempt from decision 12. That is a real possibility and the guard
...
only: decision 11's stronger requirement — a control on every page that
introduces a concept — is not derivable, because "introduces a concept" is a
reading.
...
renamed page leaves a 404 behind. Cheap, and it is what makes decision 15's URL
move safe.
...
### The journey has no guard, and that is stated rather than hidden

Every other decision in this document has machinery behind it. Executed
examples have G3 and G5, a demonstration page has G2 and G4, the ordering files
have G1, the domain has G9, the length has G8, the links have G7. Decisions 1
to 3 have nothing. The cumulative-order rule is a rule a writer follows and a
reviewer checks, and a section whose order teaches badly will build clean.

A guard over it was considered and rejected. The obvious shape is a term list
per teaching page, checked so that no page uses a term or an API no earlier page
...
The bottom four are the ones that will quietly rot, and knowing which they are
...
2. **Decision 15**: `acl/integrations/` becomes a `Platforms` band. Before
   `@evanion/acl` publishes, which is what makes the URL change free.
3. **The two front-page demos into the shop** (section 6), then decision 17 and
...
4. **G5 and G7.** Both cheap, both catch real breakage, neither needs content
   to change first.
5. **Decision 14**: `// #region` in source files. One parser change in
   `tools/doc-examples/src/regions.mjs`, plus the `fails-type-check` tag.
6. **`compose`** (section 10), as the pilot for the whole standard. It is the
...
8. **`acl` into the shop**, and G9 behind it. Nineteen regions and 22 pages,
   ...
   look like.
9. **Decision 18**: pick a declaration-to-MDX generator and put `acl`'s API
...
10. **`astro-widget`** (section 11), last, because its control is the one that
...
- **That an unenforced rule holds.** Decisions 1 to 3 are conventions, by the
  ruling in section 12. Every other convention this repo has kept has a guard
  behind it, and the ones that do not have historically drifted. The mitigation
  is that the order is visible in one file per section, so the review is a file
...
- **That `astro-widget`'s prebuilt-variant control is buildable.** Section 11
  ...
  they cannot, `astro-widget` takes the exemption and decision 12 covers eight of
...
- **Decision 18's generator.** `acl`'s API reference fences are grouped

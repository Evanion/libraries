...
definite outcome beats an unevaluable one; among definite outcomes, deny beats
allow:
...
UNDECIDABLE if no rule matches and at least one is unevaluable, and FAILS
otherwise.
...
- **A rule is unevaluable only when every condition that does not read the
  ...
  unevaluable. This is what makes the spec's mixed-branch case come out
  `allow`: `or(eq('object.authorId', 'subject.id'), always)` with no instance
  allows on the `always` branch.

A deny rule denies only on a definite match; an unevaluable deny does not deny.
 ...
 * The rule is unevaluable only if every remaining condition holds: one false
...
function ruleOutcome(
  ...
  if (missing.length > 0) {
    return { state: 'unevaluable', rule: id, missing: [...new Set(missing)] };
  }
...
function rulesAllow(permission: Permission, ctx: EvaluationContext): Decision {
  ...
  for (const [index, rule] of (permission.rules ?? []).entries()) {
    ...
    if (outcome.state === 'unevaluable') undecided.push(...outcome.missing);
  }
 ...
 * 3. the allow side definitely fails -> no-rule-matched
 * 4. the deny side is unevaluable -> unevaluable
 * 5. an allow rule matches -> allow
...
`path` operand only, treat a false object-independent condition as unevaluable,

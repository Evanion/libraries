 ...
 */
export class KeyMismatchError extends AuthorizationConfigError {
  readonly key: string;
 ...
 */
export class NotSerializableError extends AuthorizationConfigError {
  readonly key: string;
 ...
 * AuthorizationConfigError subclass.
 */
...
Field rules are leaf-level and never cascade, so `canFields` needs no resolved
map.
...
export function createPolicy(
  ...
  const canFields = (
    ...
    // Field rules are leaf-level: they never cascade through dependsOn.
    return decideFields(
      permission,
...
- **Open question carried from the spec:** the error hierarchy is prefixed
  `Authorization` while the package is `@evanion/acl`. Renaming the base to
  `AclConfigError` is a breaking change to a public export, so it is settled
  before the first publish or not at all. The plan builds `AuthorizationConfigError`;
  the rename is a decision for the user, not for a task.

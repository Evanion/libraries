...
- a condition `field` or `path` outside `subject.*` / `object.*` / `now`. This is
  a namespace check: field-name validity is the typed path's compile-time
  guarantee (Task 10), and a foreign matrix has no types to check against.
- a node that does not round-trip through JSON — a function, a `Date`, a `Map`, a
  class instance, a captured object reference → `NotSerializableError`. The
  matrix ships to the client as JSON, so a node that survives construction but
  not serialization hands the two runtimes different matrices.
